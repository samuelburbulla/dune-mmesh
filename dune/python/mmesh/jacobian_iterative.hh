...
class ParallelizedScalarProduct : public ScalarProduct<X> {
 public:
  ...
  virtual real_type norm(const X& x) const {
    u.blockVector() = x[_0];
    t.blockVector() = x[_1];
    return std::sqrt(u.scalarProductDofs(u) +
                     t.scalarProductDofs(t));  // already parallel
  }
...
class ParallelizedMatrixAdapter : public AssembledLinearOperator<M, X, Y> {
 public:
  ...
  explicit ParallelizedMatrixAdapter(const M& A, const BulkDF& uh,
  ...
  //! get matrix via *
  const M& getmat() const override { return *_A_; }
...
class Jacobian {
 ...
 public:
  ...
  using ParameterType = Fem::NewtonParameter<
      typename IScheme::LinearInverseOperatorType::SolverParameterType>;
  ...
  void solve(const Solution& f, const ISolution& g, Solution& u, ISolution& t) {
    auto setBlock = [this](const auto& block, const auto blockrow,
                           const auto blockcol) {
      this->M_[blockrow][blockcol] = block.exportMatrix();
    };

    if (n_ > 0) setBlock(A_, _0, _0);
    if (n_ > 0 && m_ > 0) {
      setBlock(B_, _0, _1);
      setBlock(C_, _1, _0);
    }
    if (m_ > 0) setBlock(D_, _1, _1);

    b_[_0] = f.blockVector();
    b_[_1] = g.blockVector();

    auto params = std::make_shared<Fem::ISTLSolverParameter>(
        ParameterType(scheme_.parameter()).linear());
    ...
    ParallelizedMatrixAdapter<BlockMatrix, BlockVector, BlockVector, Solution,
                              ISolution>
        linop(M_, u, t);
