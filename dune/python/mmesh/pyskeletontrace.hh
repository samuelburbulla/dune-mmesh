...
struct SkeletonGF
    : public BindableGridFunctionWithSpace<
          FemPy::GridPart<BulkGV>, typename InterfaceGridFunction::RangeType> {
  using GridView = BulkGV;
  using GridPart = FemPy::GridPart<BulkGV>;
  using Base =
      BindableGridFunctionWithSpace<GridPart,
  ...
  static constexpr bool scalar =
      (InterfaceGridFunction::RangeType::dimension == 1);

  SkeletonGF(const BulkGV &bulkGV, const InterfaceGridFunction &igf)
      : Base(FemPy::gridPart<BulkGV>(bulkGV), "skeleton_" + igf.name(),
             igf.order()),
        ilf_(igf),
        onInterface_(false) {}

  void bind(const typename Base::EntityType &entity) { Base::bind(entity); }
  ...
  void bind(const typename BulkGV::Intersection &intersection,
    ...
    if (Base::gridPart().grid().isInterface(intersection)) {
      ...
      sideGeometry_ = (side == IntersectionSide::in)
                          ? intersection.geometryInInside().impl()
                          : intersection.geometryInOutside().impl();
    } else
      onInterface_ = false;
  }
...
template <class BulkGV, class InterfaceGridFunction>
inline static void registerSkeletonGF(
    pybind11::module module,
    pybind11::class_<SkeletonGF<BulkGV, InterfaceGridFunction> > cls) {
  using pybind11::operator""_a;

  cls.def(pybind11::init(
              [](const BulkGV &bulkGV, const InterfaceGridFunction &bgf) {
                return SkeletonGF<BulkGV, InterfaceGridFunction>(bulkGV, bgf);
              }),
          "bulkGV"_a, "igf"_a, pybind11::keep_alive<1, 2>(),
...
struct TraceGF
    : public Dune::Fem::BindableGridFunctionWithSpace<
          Dune::FemPy::GridPart<IGV>, typename BulkGridFunction::RangeType> {
  using GridView = IGV;
  using GridPart = Dune::FemPy::GridPart<IGV>;
  using Base = Dune::Fem::BindableGridFunctionWithSpace<
      GridPart, typename BulkGridFunction::RangeType>;
  ...
  using SideGeometry = typename GridPart::GridType::MMeshType::Intersection::
      LocalGeometry::Implementation;
  static constexpr bool scalar = (BulkGridFunction::RangeType::dimension == 1);

  TraceGF(const IGV &iGV, const BulkGridFunction &bgf)
      : Base(Dune::FemPy::gridPart<IGV>(iGV), "trace_" + bgf.name(),
             bgf.order()),
  ...
  void bind(const typename Base::EntityType &entity) {
    ...
    const auto intersection =
        Base::gridPart().grid().getMMesh().asIntersection(entity);
  ...
  template <class Point>
  void evaluate(const Point &x, typename Base::RangeType &ret) const {
    // again need to transfer the x (in this case on the interface) to an x in
    // bulk
    auto xLocal = Dune::Fem::coordinate(x);
  ...
  void jacobian(const Point &x, typename Base::JacobianRangeType &ret) const {
    ...
    ret = bulkJac;  // could decide to remove normal component - but perhaps
                    // don't have to?
  }
...
template <class IGV, class BulkGridFunction, Dune::Fem::IntersectionSide side>
inline static void registerTraceGF(
    pybind11::module module,
    pybind11::class_<TraceGF<IGV, BulkGridFunction, side> > cls) {
  using pybind11::operator""_a;

  cls.def(pybind11::init([](const IGV &iGV, const BulkGridFunction &bgf) {
            return TraceGF<IGV, BulkGridFunction, side>(iGV, bgf);
          }),
          "iGV"_a, "bgf"_a, pybind11::keep_alive<1, 2>(),
          pybind11::keep_alive<1, 3>());

  cls.def_property_readonly(
      "scalar",
      [](TraceGF<IGV, BulkGridFunction, side> &self) { return self.scalar; });
