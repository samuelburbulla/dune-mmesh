 ...
 * \brief   Class defining an indicator for grid remeshing regarding the ratio
 * of outer to inner radius.
 */
...
/*!
 * \ingroup MMesh Adaptive
 * \brief   Class defining an indicator for grid remeshing regarding the edge
 * length ratio. By default, we take 2x length of the longest edge contained in
 * the interface as maximal edge length and 0.5x length of the shortest edge as
 * minimal edge length.
 */
template <class Grid>
class RatioIndicator {
 ...
 public:
   ...
   *
   * \param h                The objective edge length (aims at edge length in
   * [h/4, 2*h]). \param distProportion   Cells with distance to interface of
   * value greater than distProportion * max(dist) are refined to ... \param
  ...
  //! Calculates minH_ and maxH_ for the current interface edge length and sets
  //! factor_ to maxh / minh.
