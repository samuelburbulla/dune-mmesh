 ...
 *   \ingroup MMesh
 *  The caching entity copys the CGAL face object instead of holding a
 * Face_handle pointer.
 *
 *  A Grid is a container of grid entities. An entity is parametrized by the
 * codimension. An entity of codimension c in dimension d is a d-c dimensional
 * object.
 *
 */
...
class MMeshCachingEntity<0, dim, GridImp>
 ...
 public:
  ...
  explicit MMeshCachingEntity(const GridImp* mMesh,
                              const HostGridEntity& hostEntity)
      : BaseType(mMesh, hostEntity,
                 mMesh->globalIdSet().id(mMesh->entity(hostEntity))) {
    const auto geo = mMesh->entity(hostEntity).geometry();
    for (int i = 0; i < dim + 1; ++i) this->vertex_[i] = geo.corner(i);
  }
  ...
  bool hasFather() const { return true; }

  //! returns true if this entity is new after adaptation
  const bool isNew() const { return false; }

  //! returns true if this entity will vanish after adaptation
  const bool mightVanish() const { return true; }
  ...
  template <int d = dim>
  std::enable_if_t<d == 2, ctype> intersectionVolume(
      const MMeshEntityType& entity) const {
    ...
    return std::abs(
        PC::polygonArea(PC::sutherlandHodgman(this->vertex_, entityPoints)));
  }

  template <int d = dim>
  std::enable_if_t<d == 3, ctype> intersectionVolume(
      const MMeshEntityType& entity) const {
