...
struct DGFGridFactory<MMesh<HostGrid, dim> > {
  ...
  std::vector<double> &parameter(const Element &element) {
    if (numParameters<0>() <= 0) {
      DUNE_THROW(InvalidStateException,
                 "Calling DGFGridFactory::parameter is only allowed if there "
                 "are parameters.");
    }
  ...
  bool haveBoundaryParameters() const { return dgf_.haveBndParameters; }

  template <class GG, class II>
  const DGFBoundaryParameter::type boundaryParameter(
      const Intersection<GG, II> &intersection) const {
 ...
 private:
  bool generate(std::istream &input) {
    ...
    if (!dgf_.readDuneGrid(input, dimension, dimension)) return false;

    // Insert vertices
    for (int n = 0; n < dgf_.nofvtx; ++n) {
      typename GridFactory::WorldVector coord;
      for (std::size_t i = 0; i < dimension; ++i) coord[i] = dgf_.vtx[n][i];
      factory_.insertVertex(coord);
    }

    // Insert elements
    for (int n = 0; n < dgf_.nofelements; ++n)
      factory_.insertElement(GeometryTypes::simplex(dimension),
                             dgf_.elements[n]);
...
template <class HostGrid, int dim>
inline DGFGridFactory<MMesh<HostGrid, dim> >::DGFGridFactory(
    std::istream &input, MPICommunicatorType comm)
    : dgf_(0, 1) {
  input.clear();
  input.seekg(0);
