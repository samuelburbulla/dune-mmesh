...
static inline FieldVector<typename Kernel::RT, 2> makeFieldVector(
 ...
 */
template <class Kernel>
static inline FieldVector<typename Kernel::RT, 3> makeFieldVector(
...
auto computeCircumcenter(const Geometry& geo) {
  ...
  if constexpr (mydim == 3) {
    ...
    return makeFieldVector(
        CGAL::circumcenter(makePoint(geo.corner(0)), makePoint(geo.corner(1)),
                           makePoint(geo.corner(2)), makePoint(geo.corner(3))));
  }

  if constexpr (mydim == 2) {
    // obtain circumcenter by CGAL
    return makeFieldVector(CGAL::circumcenter(makePoint(geo.corner(0)),
                                              makePoint(geo.corner(1)),
                                              makePoint(geo.corner(2))));
  }

  if constexpr (mydim == 1) return 0.5 * (geo.corner(0) + geo.corner(1));

  if constexpr (mydim == 0) return geo.corner(0);
}
