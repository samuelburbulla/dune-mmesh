...
#ifndef DUNE_MMESH_GRID_GMSHPARSER_HH
#define DUNE_MMESH_GRID_GMSHPARSER_HH
...
namespace Dune
{
  ...
  private:
    bool generate( std::istream &gridFile )
    {
      ...
      // read file until we get to the list of elements, then print a warning that elements will not be handled
      while( !gridFile.eof() )
      {
        if( line.find("$Elements") != std::string::npos )
        {
          std::cerr << "Warning: MMesh Gmsh reader does not support reading of elements, because triangles are generated by CGAL. The elements block will be ignored!" << std::endl;
          break;
        }
        std::getline(gridFile, line);
      }
