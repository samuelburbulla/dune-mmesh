...
{
  static_assert(Entity::mydimension <= 1,
                "Incident element range iterator is only available for "
                "vertices and edges!");
  return IteratorRange<decltype(entity.impl().incidentBegin())>(
      entity.impl().incidentBegin(), entity.impl().incidentEnd());
}
...
{
  static_assert(
      Vertex::mydimension == 0,
      "Incident facet range iterator is only available for vertices!");
  return IteratorRange<decltype(vertex.impl().incidentFacetsBegin())>(
      vertex.impl().incidentFacetsBegin(), vertex.impl().incidentFacetsEnd());
}
...
#endif /* DOXYGEN_SHOULD_SKIP_THIS */
{
  ...
  return IteratorRange<decltype(vertex.impl().incidentVerticesBegin(
      includeInfinite))>(vertex.impl().incidentVerticesBegin(includeInfinite),
...
#endif /* DOXYGEN_SHOULD_SKIP_THIS */
{
  return IteratorRange<decltype(gv.grid().template interfaceBegin<codim>(
      includeBoundary))>(
...
#endif /* DOXYGEN_SHOULD_SKIP_THIS */
{
  return IteratorRange<decltype(gv.grid().interfaceVerticesBegin(
      includeBoundary))>(gv.grid().interfaceVerticesBegin(includeBoundary),
...
#endif /* DOXYGEN_SHOULD_SKIP_THIS */
{
  static_assert(Vertex::mydimension == 0,
                "Incident interface vertices range iterator is only available "
                "for interface vertices!");
...
#endif /* DOXYGEN_SHOULD_SKIP_THIS */
{
  static_assert(Entity::mydimension <= 1,
                "Incident interface vertices range iterator is only available "
                "for interface vertices and edges!");
  return IteratorRange<
