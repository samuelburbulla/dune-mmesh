...
struct MMeshStructuredGridFactory {
  ...
  explicit MMeshStructuredGridFactory(
      const GlobalPosition& lowerLeft, const GlobalPosition& upperRight,
  ...
  //! return grid pointer
  const std::shared_ptr<Grid> grid() const { return grid_; }
 ...
 public:
   ...
   */
  template <long unsigned int dim>
  static typename Grid::GridPtrType createStructuredGrid(
      const GlobalPosition& lowerLeft, const GlobalPosition& upperRight,
      const std::array<unsigned int, dim>& elements) {
    // The grid factory
    GridFactory factory;
    ...
    for (int i = 0; i < numVertices; i++, ++index) {
      ...
      for (std::size_t j = 0; j < dim; j++)
        pos[j] = lowerLeft[j] +
                 index[j] * (upperRight[j] - lowerLeft[j]) / (vertices[j] - 1);
      for (std::size_t j = dim; j < dimworld; j++) pos[j] = lowerLeft[j];
