...
using MMeshInterfaceIterator =
    EntityIterator<codim, Grid,
                   MMeshInterfaceIteratorImp<codim, Grid, Grid::dimension>>;
...
class MMeshInterfaceIteratorImp<1, GridImp, 2> {
 ...
 public:
   ...
   * constructor \param mMesh  pointer to grid instance
   */
  explicit MMeshInterfaceIteratorImp(const GridImp* mMesh, bool endDummy,
  ...
  //! dereferencing
  Entity dereference() const { return Entity{{mMesh_, *edgeIterator_}}; }
 ...
 private:
  bool isInterface_() const {
    if (includeBoundary_)
      ...
      return mMesh_->isInterface(dereference()) &&
             !mMesh_->getHostGrid().is_infinite(
                 edgeIterator_->first)  // exclude boundary segments
             && !mMesh_->getHostGrid().is_infinite(
                    edgeIterator_->first->neighbor(edgeIterator_->second));
    }
  }
...
class MMeshInterfaceIteratorImp<1, GridImp, 3> {
 ...
 public:
  ...
  //! dereferencing
  Entity dereference() const { return Entity{{mMesh_, *facetIterator_}}; }
 ...
 private:
  bool isInterface_() const {
    if (includeBoundary_)
      ...
      return mMesh_->isInterface(dereference()) &&
             !mMesh_->getHostGrid().is_infinite(
                 facetIterator_->first)  // exclude boundary segments
             && !mMesh_->getHostGrid().is_infinite(
                    facetIterator_->first->neighbor(facetIterator_->second));
    }
  }
...
//!  The Entity Iterator alias
template <class Grid>
using MMeshInterfaceVertexIterator =
    EntityIterator<Grid::dimension, Grid,
                   MMeshInterfaceVertexIteratorImp<Grid, Grid::dimension>>;
...
class MMeshInterfaceVertexIteratorImp {
 ...
 public:
  ...
  explicit MMeshInterfaceVertexIteratorImp(const GridImp* mMesh,
                                           bool includeBoundary)
      : mMesh_(mMesh),
   ...
   * constructor \param mMesh  pointer to grid instance
   */
  explicit MMeshInterfaceVertexIteratorImp(const GridImp* mMesh, bool endDummy,
                                           bool includeBoundary)
      : mMesh_(mMesh),
        interfaceIterator_(
            InterfaceIteratorImpl(mMesh_, true, includeBoundary)),
        interfaceEnd_(InterfaceIteratorImpl(mMesh_, true, includeBoundary)),
        i_(0) {}
  ...
  Vertex dereference() const {
    ...
    return Vertex{
        {mMesh_, host.first->vertex((host.second + i_ + 1) % (dim + 1))}};
  }
