...
/**
 * \brief The EntitySeed class provides the minimal information needed to
 * restore an Entity using the grid. \ingroup MMesh We use the host grid entity
 * as seed.
 */
...
class MMeshEntitySeed {
 ...
 public:
   ...
   */
  MMeshEntitySeed(const HostGridEntity& hostEntity) : hostEntity_(hostEntity) {}

  /**
   * \brief Construct a seed from another seed.
   */
  MMeshEntitySeed& operator=(const MMeshEntitySeed<codim, GridImp>& seed) {
    if (this != &seed) hostEntity_ = seed.hostEntity_;
    return *this;
  }
