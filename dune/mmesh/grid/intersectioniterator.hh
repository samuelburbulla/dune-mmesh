...
class MMeshLeafIntersectionIterator {
 ...
 public:
  typedef Dune::Intersection<const GridImp,
                             Dune::MMeshLeafIntersection<GridImp> >
      Intersection;
  ...
  MMeshLeafIntersectionIterator(const GridImp* mMesh,
      ...
      : mMesh_(mMesh), hostEntity_(hostEntity), i_(0) {
    while (proceed()) increment();
  }

  //! constructor for end iterator
  MMeshLeafIntersectionIterator(const GridImp* mMesh,
                                const HostGridEntity& hostEntity, bool endDummy)
      : mMesh_(mMesh), hostEntity_(hostEntity), i_(dim + 1) {}
  ...
  Intersection dereference() const {
    // remark: the i-th intersection in CGAL corresponds to the (dim-i)-th edge
    // in DUNE
 ...
 private:
  ...
  bool proceed() {
    ...
    if (hostEntity_->info().partition == 2) {
      ...
      if (mMesh_->getHostGrid().is_infinite(neighborHostEntity)) return true;

      // skip ghost neighbors
      if (neighborHostEntity->info().partition != 0) return true;
    }
    return false;
  }
