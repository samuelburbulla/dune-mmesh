...
class MMeshExplicitGridFactory : public GridFactoryInterface<Grid> {
 ...
 public:
   ...
   *
   *  \param[in]  type          GeometryType of the new element
   *  \param[in]  v             indices of the element vertices (starting with
   * 0) \param[in]  domainMarker  domain marker of element
   */
  void insertElement(const GeometryType& type,
                     const std::vector<unsigned int>& v,
 ...
 private:
  ...
  /** \brief Creates an element (face) in the underlying triangulation data
   * structure \ingroup 2D
   *
  ...
  /** \brief Creates an element (cell) in the underlying triangulation data
   * structure \ingroup 3D
   *
   *  \param[in]  v     indices of the element vertices
   */
  template <int d = dimension>
  ...
  /** \brief Adds the facet index tuples as set into a map to find neighbor
   * relations
   *
   *  \param[in]  v        indices of the facet vertices
   *  \param[in]  element  the element handle where facet belongs to
   *  \param[in]  fi       the index of the facet (= index of neighbor) in
   * element
   */
 ...
 public:
  /** \brief Returns if there is a face with the given vertices in the
   * triangulation \ingroup 2D
   *
   *  \param[in]  v        indices of the element vertices
   */
  template <int d = dimension>
  ...
  /** \brief Returns if there is a cell with the given vertices in the
   * triangulation \ingroup 3D
   *
   *  \param[in]  v        indices of the element vertices
   */
  template <int d = dimension>
