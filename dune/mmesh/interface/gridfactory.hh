...
class GridFactory<MMeshInterfaceGrid<MMeshImp> >
 ...
 public:
  ...
  typedef std::unordered_map<std::vector<std::size_t>, std::size_t,
                             HashUIntVector>
      BoundarySegments;
 ...
 public:
  ...
  /** default constructor */
  GridFactory(const std::shared_ptr<MMesh> mMesh) : mMesh_(mMesh) {}
  ...
  void insertElement(const GeometryType &type,
    ...
    for (const auto &v : vertices) ids.push_back(vertexIdMap_.at(v));
  ...
  /** \brief Add existing vertex handle from the macro grid to the interface
   * grid
   *
  ...
  unsigned int insertionIndex(const typename Codim<0>::Entity &entity) const {
    ...
    for (std::size_t i = 0; i < entity.subEntities(dimension); ++i)
      ids.push_back(entity.template subEntity<dimension>(i)
                        .impl()
                        .hostEntity()
                        ->info()
                        .id);
