...
static inline auto computeCGALIndices(const HostEntity& hostEntity) {
  ...
  for (std::size_t i = 0; i < dim + 1; ++i) indices[i] = pairs[i].first;

  assert(hostEntity.first->vertex(indices[0])->info().id <
         hostEntity.first->vertex(indices[1])->info().id);

  if constexpr (dim == 2)
    assert(hostEntity.first->vertex(indices[1])->info().id <
           hostEntity.first->vertex(indices[2])->info().id);
