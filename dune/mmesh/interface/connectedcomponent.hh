...
class MMeshInterfaceConnectedComponent {
 ...
 private:
  ...
  using CachingEntity =
      MMeshInterfaceCachingEntity<0, GridImp::dimension, const GridImp>;
 ...
 public:
  typedef MMeshInterfaceGridGeometry<GridImp::dimension, GridImp::dimension + 1,
                                     GridImp>
      Geometry;
  ...
  void add(const Element& element) {
    ...
    assert(children_.size() <=
           2);  // at the moment, more children are not supported
  }

  //! Return list of caching entities in this component
  const std::vector<CachingEntity>& children() const { return children_; }

  //! Return number of caching entities in this component
  const std::size_t size() const { return children_.size(); }
