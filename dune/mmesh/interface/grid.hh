...
struct MMeshInterfaceGridFamily {
 public:
  typedef GridTraits<
      ...
      MMeshInterfaceGrid<MMesh>, MMeshInterfaceGridGeometry,
      ...
      MMeshInterfaceGridHierarchicIterator, MMeshInterfaceGridLeafIterator,
      MMeshInterfaceGridLeafIndexSet<
          const MMeshInterfaceGrid<MMesh>>,  // LevelIndexSet
      ...
      MMeshInterfaceGridGlobalIdSet<
          const MMeshInterfaceGrid<MMesh>>,  // LocalIdSet
      MMeshImpl::MultiId,                    // LocalIdSet::IdType,
      Communication<Comm>, DefaultLevelGridViewTraits,
      DefaultLeafGridViewTraits, MMeshInterfaceGridEntitySeed>
      Traits;
};
...
//************************************************************************
/*!
 * \brief Provides a DUNE grid interface class for the interface of a MMesh
 * interface grid \ingroup GridImplementations \ingroup MMeshInterfaceGrid
 */
template <class MMesh>
...
#endif /* DOXYGEN_SHOULD_SKIP_THIS */
{
 public:
  ...
  template <int cd>
  using MMeshInterfaceEntity =
      typename HostGridEntityChooser_<HostGridType, dimensionworld,
                                      cd + 1>::type;
  ...
  //! Constructor
  explicit MMeshInterfaceGrid(MMesh* mMesh,
  ...
  {
    leafIndexSet_ =
        std::make_unique<MMeshInterfaceGridLeafIndexSet<const GridImp>>(this);
    globalIdSet_ =
        std::make_unique<MMeshInterfaceGridGlobalIdSet<const GridImp>>(this);
  }
  ...
  void addBoundarySegment(const std::vector<std::size_t>& ids,
  ...
  const MMeshInterfaceGridLeafIndexSet<const GridImp>& levelIndexSet(
      int level) const {
    if (level != 0)
  ...
  typename Traits::template Codim<EntitySeed::codimension>::Entity entity(
    ...
    typedef MMeshInterfaceGridEntity<EntitySeed::codimension, dimension,
  ...
  typename Traits::template Codim<dimension>::Entity entity(
      const MMeshInterfaceEntity<dimension>& hostEntity) const {
    return entity(
        typename Traits::template Codim<dimension>::EntitySeed(hostEntity));
  }

  /** \brief Create codim 0 entity from a host entity */
  typename Traits::template Codim<0>::Entity entity(
      const MMeshInterfaceEntity<0>& hostEntity) const {
    return entity(typename Traits::template Codim<0>::EntitySeed(hostEntity));
  }

  /** \brief Create codim 0 entity from a host entity */
  template <int dimw = dimension + 1>
  std::enable_if_t<dimw == 3, typename Traits::template Codim<1>::Entity>
  entity(const MMeshInterfaceEntity<1>& hostEntity) const {
  ...
  typename Traits::template Codim<codim>::LevelIterator lbegin(
      int level) const {
    return MMeshInterfaceGridLeafIterator<codim, All_Partition, const GridImp>(
        this);
  }
  ...
  bool preAdapt() { return mark_.size() > 0; }
   ...
   */
  bool adapt() { return mMesh_->adapt(); }
 ...
 public:
  ...
  unsigned int ghostSize(int codim) const {
    ...
    if (codim == 0)
      for (const auto& e : elements(this->leafGridView()))
        if (e.partitionType() == GhostEntity) size++;
  ...
  /** \brief Distributes this grid over the available nodes in a distributed
   * machine
   *
   * \param minlevel The coarsest grid level that gets distributed
   * \param maxlevel does currently get ignored
   */
  void loadBalance() { mMesh_->loadBalance(); };
  ...
  void communicate(Data& dataHandle, InterfaceType interface,
    ...
    if ((interface == InteriorBorder_All_Interface) ||
      ...
      MMeshCommunication<GridImp, MMeshType> communication(
          getMMesh().partitionHelper());
  ...
  std::enable_if_t<d == 2, bool> isInterface(
      const MMeshInterfaceEntity<1>& edge) const {
    auto circulator = mMesh_->getHostGrid().incident_facets(edge);
    for (std::size_t i = 0; i < CGAL::circulator_size(circulator);
         ++i, ++circulator)
      if (isInterface(*circulator)) return true;
    return false;
  }
  ...
  bool isInterface(
      const typename Traits::LeafIntersection& intersection) const {
  ...
  std::size_t domainMarker(const Entity& entity) const {
    ...
    return mMesh_
        ->interfaceSegments()[getVertexIds_(entity.impl().hostEntity())];
  }

  //! Mark a set of children elements as refinement of a connected component
  void markAsRefined(const std::vector<std::vector<std::size_t>>& children,
  ...
  bool hasConnectedComponent(const Entity& entity) const {
    int count = childrenConnectedComponentMap_.count(
        getVertexIds_(entity.impl().hostEntity()));
    assert(count <= 1);
    return (count == 1);
  }
  ...
  const ConnectedComponent& getConnectedComponent(const Entity& entity) const {
    auto it = childrenConnectedComponentMap_.find(
        getVertexIds_(entity.impl().hostEntity()));
  ...
  std::enable_if_t<d == 2, const MMeshInterfaceEntity<0>> mirrorHostEntity(
      const MMeshInterfaceEntity<0>& other) const {
    return mMesh_->getHostGrid().mirror_edge(other);
  }

  //! Mirror a host entity (3d)
  template <int d = dimensionworld>
  std::enable_if_t<d == 3, const MMeshInterfaceEntity<0>> mirrorHostEntity(
      const MMeshInterfaceEntity<0>& other) const {
 ...
 private:
  ...
  static inline auto getVertexIds_(const MMeshInterfaceEntity<0>& entity) {
    ...
    for (int i = 0; i < dimensionworld; ++i)
      ids[i] =
          entity.first->vertex((entity.second + i + 1) % (dimensionworld + 1))
              ->info()
              .id;
 ...
 public:
  ...
  const MMesh& getMMesh() const { return *mMesh_; }

  //! Return reference to underlying CGAL triangulation
  const HostGridType& getHostGrid() const { return mMesh_->getHostGrid(); }

  //! Return sequence number
  int sequence() const { return sequence_; }

  const auto& partitionHelper() const { return mMesh_->partitionHelper(); }
 ...
 private:
  ...
  std::unordered_map<std::vector<std::size_t>, ConnectedComponent,
                     HashUIntVector>
      childrenConnectedComponentMap_;
...
struct DGFGridInfo<MMeshInterfaceGrid<MMesh>> {
  static int refineStepsForHalf() { return MMesh::dimension - 1; }
