...
class Plane {
 public:
  ...
  auto signedDistance(const Coordinate& x) const { return n_ * (x - p_); }
...
template <>
class TriangulationWrapper<2> : public MMeshDefaults::Triangulation<2>::type {
 ...
 public:
  ...
  template <class VertexHandle, class Elements>
  void remeshHoleConstrained(const VertexHandle& vh,
                             std::list<FacetHandle>& hole, Elements& elements,
 ...
 private:
  Vertex_pair make_vertex_pair(FacetHandle f) const {
    ...
    if (vp.first > vp.second) std::swap(vp.first, vp.second);
...
template <>
class TriangulationWrapper<3> : public MMeshDefaults::Triangulation<3>::type {
...
template <int dim>
class DelaunayTriangulationWrapper : public MMeshDefaults::Delaunay<dim>::type {
