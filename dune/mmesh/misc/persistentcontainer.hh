...
class PersistentContainer<MMesh<HostGrid, dim>, T>
    : public Dune::PersistentContainerMap<
          MMesh<HostGrid, dim>, typename MMesh<HostGrid, dim>::LocalIdSet,
          std::unordered_map<typename MMesh<HostGrid, dim>::LocalIdSet::IdType,
                             T> > {
  typedef MMesh<HostGrid, dim> G;
  typedef PersistentContainerMap<
      G, typename G::LocalIdSet,
      std::unordered_map<typename G::LocalIdSet::IdType, T> >
      Base;
  typedef typename MMesh<HostGrid, dim>::LocalIdSet IdSet;
  typedef std::unordered_map<typename MMesh<HostGrid, dim>::LocalIdSet::IdType,
                             T>
      Map;
 ...
 public:
  ...
  void resize(const Value &value = Value()) {
    if (sequence_ != this->grid_->sequence()) {
      Hybrid::forEach(std::make_index_sequence<Grid::dimension + 1>{},
                      [&](auto i) {
                        if (i == this->codimension())
                          this->template resize<i>(value);
                      });
  ...
  void resize(const Value &value) {
    std::integral_constant<bool, Capabilities::hasEntity<Grid, codim>::v>
        hasEntity;
    assert(codim == codimension());

    // add one id for caching entity during adaptation
    if constexpr (codim == 0) {
      MMeshImpl::MultiId id(
...
class PersistentContainer<MMeshInterfaceGrid<MMesh>, T>
    : public Dune::PersistentContainerMap<
          ...
          std::unordered_map<
              typename MMeshInterfaceGrid<MMesh>::LocalIdSet::IdType, T> > {
