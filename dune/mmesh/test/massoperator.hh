...
class MassOperator : public LinearOperator {
  ...
  typedef Dune::Fem::CachingQuadrature<
      typename DiscreteFunctionSpaceType::GridPartType, 0>
      QuadratureType;

 public:
  explicit MassOperator(const DiscreteFunctionSpaceType &dfSpace)
      : BaseType("mass operator", dfSpace, dfSpace), dfSpace_(dfSpace) {
    assemble(*this);
  }
...
class AffineMassOperator
 ...
 public:
  ...
  template <class Function>
  explicit AffineMassOperator(const DiscreteFunctionSpaceType &dfSpace,
                              const Function &u)
  ...
  virtual void jacobian(const DiscreteFunctionType &u,
...
template <class DiscreteFunction, class LinearOperator>
template <class Function>
void MassOperator<DiscreteFunction, LinearOperator>::assembleRHS(
    const Function &u, DiscreteFunctionType &w) const {
  // clear result
  w.clear();
  ...
  for (const auto &entity :
       elements(dfSpace_.gridPart(), Dune::Partitions::interiorBorder)) {
    ...
    for (const auto qp :
         QuadratureType(entity, uLocal.order() + wLocal.order())) {
      // evaluate u
...
void MassOperator<DiscreteFunction, LinearOperator>::assemble(
    LinearOperator &matrix) const {
  matrix.reserve(Dune::Fem::DiagonalStencil<DiscreteFunctionSpaceType,
                                            DiscreteFunctionSpaceType>(
      dfSpace_, dfSpace_));
  matrix.clear();
