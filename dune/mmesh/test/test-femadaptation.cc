...
struct DataOutputParameters
    : public Dune::Fem::LocalParameter<Dune::Fem::DataOutputParameters,
                                       DataOutputParameters> {
  DataOutputParameters(const int step) : step_(step) {}
...
struct IDataOutputParameters
    : public Dune::Fem::LocalParameter<Dune::Fem::DataOutputParameters,
                                       IDataOutputParameters> {
  IDataOutputParameters(const int step) : step_(step) {}
...
struct Scheme {
...
#else
  typedef Dune::Fem::DiscontinuousGalerkinSpace<FunctionSpaceType, GridPartType,
                                                POLORDER>
      DiscreteFunctionSpaceType;
#endif
  ...
  typedef Dune::Fem::RestrictProlongDefault<DiscreteFunctionType>
      RestrictionProlongationType;

  typedef Dune::Fem::AdaptationManager<GridType, RestrictionProlongationType>
  ...
  bool mark(double time) const { return grid_.markElements(); }
 ...
 protected:
  GridPartType &gridPart_;  // grid part(view), e.g. here the leaf grid the
                            // discrete space is build with
  GridType &grid_;
  DiscreteFunctionSpaceType discreteSpace_;  // discrete function space
  DiscreteFunctionType solution_;            // the unknown
  RestrictionProlongationType
      restrictProlong_;  // local restriction/prolongation object
  AdaptationManagerType
      adaptationManager_;  // adaptation manager handling adaptation
  const int step_;
};
...
std::array<double, 2> algorithm(HGridType &grid, IGridType &igrid,
                                const int step) {
  ...
  typedef Dune::Fem::FunctionSpace<double, double, HGridType ::dimensionworld,
                                   1>
      FunctionSpaceType;
  typedef Dune::Fem::FunctionSpace<double, double, IGridType ::dimensionworld,
                                   1>
      IFunctionSpaceType;
  ...
  typedef Dune::Fem::GridFunctionAdapter<FunctionType, GridPartType>
      GridExactSolutionType;
  GridExactSolutionType gridExactSolution("exact solution", f, gridPart, 5);
  typedef Dune::Fem::GridFunctionAdapter<IFunctionType, IGridPartType>
      IGridExactSolutionType;
  IGridExactSolutionType igridExactSolution("interface exact solution", i_f,
                                            igridPart, 5);
  ...
  // output
  typedef std::tuple<const typename SchemeType::DiscreteFunctionType *,
                     GridExactSolutionType *>
      IOTupleType;
  ...
  typedef std::tuple<const typename ISchemeType::DiscreteFunctionType *,
                     IGridExactSolutionType *>
      IIOTupleType;
  ...
  std::array<double, 2> ret{
      {l2norm.distance(gridExactSolution, scheme.solution()),
    ...
    for (const auto &e : elements(gridPart))
      for (const auto &i : intersections(gridPart, e))
        if (!grid.isInterface(i)) {
          std::cout << "Add interface at " << i.geometry().center()
                    << std::endl;
          grid.addInterface(i);
          return;
        }
...
int main(int argc, char **argv) try {
  ...
  for (int i = 1; i < argc; ++i) Dune::Fem::Parameter::append(argv[i]);
