...
typedef Dune::Fem::LagrangeDiscreteFunctionSpace<FuncSpace1, GridPartType, 2>
    DiscreteFunctionSpaceType1;
typedef Dune::Fem::DiscontinuousGalerkinSpace<FuncSpace2, GridPartType, 1,
                                              CachingStorage>
    DiscreteFunctionSpaceType2;
typedef Dune::Fem::TupleDiscreteFunctionSpace<DiscreteFunctionSpaceType1,
                                              DiscreteFunctionSpaceType2>
    DiscreteFunctionSpaceType;

typedef
    typename DiscreteFunctionSpaceType ::FunctionSpaceType FunctionSpaceType;
typedef AdaptiveDiscreteFunction<DiscreteFunctionSpaceType>
    DiscreteFunctionType;
typedef DofManager<MyGridType> DofManagerType;
typedef AdaptationManager<MyGridType,
                          RestrictProlongDefault<DiscreteFunctionType> >
    AdaptationManagerType;

// ***********************************************************
// the exact solution to the problem for EOC calculation
struct ExactSolution : public Fem::Function<FunctionSpaceType, ExactSolution> {
...
void adapt(MyGridType &grid, DiscreteFunctionType &solution, int step) {
  const DiscreteFunctionType::DiscreteFunctionSpaceType &space =
      solution.space();
  ...
  if (Parameter ::verbose()) {
    ...
    std::cout << "AdaptiveLeafIndexSet.size:  " << space.indexSet().size(0)
              << std::endl;
  }

  for (int i = 0; i < count; ++i) {
    for (const auto &entity : space) grid.mark(mark, entity);
    adop.adapt();
...
// ********************************************************************
double algorithm(MyGridType &grid, DiscreteFunctionType &solution, int step,
                 int turn, int writestep) {
  ...
  // output
  Dune::Fem::VTKIO<GridPartType> vtkIO(solution.space().gridPart(),
                                       Dune::VTK::nonconforming);
  vtkIO.addVertexData(gridFunc);
  vtkIO.addVertexData(solution);
  if (writestep == 0) vtkIO.write("test-adapt-tuplespace-0");
  ...
  std::cout << "Interpolation on new grid: " << new_error << std::endl
            << std::endl;
...
int main(int argc, char *argv[]) try {
  ...
  const double eocThreshold =
      Parameter ::getValue("adapt.eocthreshold", double(0.2));

  const bool isLocallyAdaptive =
      Dune::Fem::Capabilities::isLocallyAdaptive<MyGridType>::v;
