...
template <>
bool compare(const Dune::FieldVector<double, 2>& a,
             const Dune::FieldVector<double, 2>& b) {
  return (a - b).two_norm() < 1e-12;
}

//! Helper method for checking multiple properties given in a list with verbose
//! output
template <class T>
void checkProperties(const std::string& name,
                     std::initializer_list<std::initializer_list<T> > data) {
  std::cout << "Check " << std::setfill(' ') << std::setw(30) << std::left
            << name;
  ...
  for (std::vector<T> datavector : data) {
    ...
    if (!correct) {
      ...
      std::cout << " - failed: result [" << resultValue
                << "] was expected to be [" << testValue << "]" << std::endl;
    }
    assert(correct);
  }
...
template <class T>
void checkProperty(const std::string& name, const T& resultValue,
                   const T& testValue) {
...
int main(int argc, char* argv[]) {
  try {
    ...
    checkProperty("number of iterated vertices", countVertices,
                  indexSet.size(2));

    // Test MMesh mcmgmapper for elements
    MultipleCodimMultipleGeomTypeMapper<decltype(gridView)> elementMapper(
        gridView, mcmgElementLayout());
    checkProperty("size of mcmg element mapper", elementMapper.size(), 4ul);
    unsigned int count = 0;
    for (auto e : elements(gridView))
      checkProperty("index mapped by element mapper", elementMapper.index(e),
                    count++);

    // Test MMesh mcmgmapper for vertices
    MultipleCodimMultipleGeomTypeMapper<decltype(gridView)> vertexMapper(
        gridView, mcmgVertexLayout());
    checkProperty("size of mcmg vertex mapper", vertexMapper.size(), 6ul);
    unsigned int vertexCount = 0;
    for (auto v : vertices(gridView))
      checkProperty("index mapped by vertex mapper", vertexMapper.index(v),
                    vertexCount++);
    ...
    for (auto e : elements(gridView)) {
      ...
      if (elementCount == 2) {
        ...
        // check element
        checkProperty("geometry center", geo.center(),
                      {0.66666666666666663, 0.083333333333333329});
      ...
      if (elementCount == 1) {
        ...
        checkProperties("boundary", {{is0.boundary(), false},
                                     {is1.boundary(), false},
                                     {is2.boundary(), false}});

        checkProperties("isIntersection", {{mMesh.isInterface(is0), false},
                                           {mMesh.isInterface(is1), false},
                                           {mMesh.isInterface(is2), false}});

        checkProperties("intersection normals",
                        {{is0.centerUnitOuterNormal(),
                          {0.24253562503633297, -0.97014250014533188}},
                         {is1.centerUnitOuterNormal(),
                          {-0.89442719099991586, 0.44721359549995793}},
                         {is2.centerUnitOuterNormal(),
                          {0.83205029433784372, 0.55470019622522915}}});

        checkProperties("index in inside and outside",
                        {{is2.indexInInside(), 2}, {is2.indexInOutside(), 2}});
      }
