...
struct DataHandle : CommDataHandleIF<DataHandle<Data, IndexSet>, Data> {
  using DataType = typename Data::value_type;

  DataHandle(Data& data, Data& dataCheck, const IndexSet& indexSet,
             int codim = 0)
      : data_(data),
        dataCheck_(dataCheck),
        indexSet_(indexSet),
        mycodim_(codim) {}

  bool contains(int dimension, int codim) const { return codim == mycodim_; }
  ...
  void scatter(Buffer& buffer, const Entity& entity, std::size_t size) {
    ...
    if (data_[i] != dataCheck_[i])
      DUNE_THROW(InvalidStateException,
                 "Element at (" << entity.geometry().center() << ") is wrong: "
                                << data_[i] << " != " << dataCheck_[i]);
  }

  static bool fixedSize(int dim, int codim) { return false; }
...
using DiscreteFunctionSpaceType =
    LagrangeDiscontinuousGalerkinSpace<SpaceType, GridPartType, polOrd>;
using DiscreteFunctionType =
    AdaptiveDiscreteFunction<DiscreteFunctionSpaceType>;

struct ExactSolution : public Fem::Function<SpaceType, ExactSolution> {
...
struct ProjectionAllPartitionNoComm {
  template <class FunctionType, class DiscreteFunctionType,
            class Partition = Partitions::All>
...
int main(int argc, char* argv[]) {
  ...
  GridPtr<BulkGridType> gridptr_((dim == 2) ? "grids/junction2d.msh"
                                            : "grids/plane3d.msh");
  ...
  for (const auto& e : elements(grid.leafGridView(), Partitions::all)) {
    ...
    if (e.partitionType() == InteriorEntity) data[idx] = id;
  }
  ...
  for (const auto& v : vertices(grid.leafGridView(), Partitions::all)) {
    ...
    if (v.partitionType() == InteriorEntity ||
        v.partitionType() == BorderEntity)
      dataVertex[idx] = id;
  }
  ...
  auto printData = [&](bool before = true) {
    ...
    for (const auto d : dataVertex) std::cout << d << "|  ";
  ...
  DataHandleType dataHandleVertex(dataVertex, dataVertexCheck,
                                  grid.leafIndexSet(), dimgrid);
  ...
  Dune::Timer timer;
  timer.start();
  grid.communicate(dataHandle, InteriorBorder_All_Interface,
                   ForwardCommunication);
  grid.communicate(dataHandleVertex, InteriorBorder_All_Interface,
                   ForwardCommunication);
  auto maxT = grid.comm().max(timer.elapsed());
  if (grid.comm().rank() == 0) std::cout << "Comm took " << maxT << std::endl;
  ...
  if (verbose)
    std::cout << "P[" << grid.comm().rank() << "]  start comm: " << old_error
              << std::endl;

  solution.clear();
  ProjectionAllPartitionNoComm::project<ExactSolution, DiscreteFunctionType,
                                        Partitions::InteriorBorder>(f,
                                                                    solution);
  ...
  if (verbose)
    std::cout << "P[" << grid.comm().rank() << "]  done comm: " << error
              << std::endl;

  if (std::abs(old_error - error) > 1e-10)
    DUNE_THROW(InvalidStateException,
               "Communication not working correctly on rank "
                   << grid.comm().rank() << ": " << old_error << " -> "
                   << error);
  else if (grid.comm().rank() == 0)
    std::cout << "Communication was successful and took " << timer.elapsed()
              << std::endl;
