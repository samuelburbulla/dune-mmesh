...
typedef Dune::Fem::FunctionSpace<typename GridType::ctype,
                                 typename GridType::ctype,
                                 GridType::dimensionworld, 1>
    SpaceType;
typedef Dune::Fem::LagrangeDiscreteFunctionSpace<SpaceType, GridPartType,
                                                 polOrder>
    DiscreteSpaceType;

typedef Dune::Fem::AdaptiveDiscreteFunction<DiscreteSpaceType>
    DiscreteFunctionType;
typedef Dune::Fem::SparseRowLinearOperator<DiscreteFunctionType,
                                           DiscreteFunctionType>
    LinearOperatorType;
typedef Dune::Fem::KrylovInverseOperator<DiscreteFunctionType>
    InverseOperatorType;
...
class TestGrid {
 ...
 protected:
  TestGrid(const std::string &name) : gridptr_(name) {
...
struct Function : public Dune::Fem::BindableGridFunction<GridPart, RangeType> {
  ...
  void evaluate(const Point &p, RangeType &value) const {
    ...
    value = 1.;
    for (int k = 0; k < x.dimension; ++k) value *= sin(M_PI * x[k]);
  }
  template <class Point>
  void jacobian(const Point &p,
                typename Base::JacobianRangeType &jacobian) const {
    auto x = Base::global(p);
...
struct Algorithm {
  ...
  explicit Algorithm(GridType &grid) : grid_(grid) {}
...
inline Algorithm::ErrorType Algorithm::operator()(int step) {
  ...
  if (grid_.comm().rank() == 0) std::cout << error << std::endl;
  return error;
}
  ...
  for (int step = 1; step < nrSteps; ++step) {
    ...
    if (std::abs(l2eoc - 1 - polOrder) > 0.2) {
      DUNE_THROW(Dune::InvalidStateException,
                 "EOC check of solving mass matrix system failed L2eoc "
                     << l2eoc << " " << polOrder);
    }

    if (std::abs(h1eoc - polOrder) > 0.2) {
      DUNE_THROW(Dune::InvalidStateException,
                 "EOC check of solving mass matrix system failed H1eoc "
                     << h1eoc << " " << polOrder);
    }
  }
}
