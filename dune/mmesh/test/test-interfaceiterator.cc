...
int main(int argc, char* argv[]) {
  try {
    ...
    std::cout << "-- Interface iterator test for " + std::to_string(dim) +
                     "D --"
              << std::endl;

    std::cout << "Build grid from mimesh" << dim << "d.msh file..."
              << std::endl;
    ...
    if (numberOfInterfaceSegments != expected)
      DUNE_THROW(GridError, "There are " << numberOfInterfaceSegments
                                         << " interface segments instead of "
                                         << expected << " expected!"
                                         << std::endl);
