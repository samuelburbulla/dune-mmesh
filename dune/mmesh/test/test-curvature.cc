...
template <class Lengths, class GridFiles, class NumOfIElems,
          class ErrorsCurvature, class ErrorsCenter>
void output(const Lengths& characteristicLengths, const GridFiles& gridfiles,
  ...
  for (int i = 0; i < numOfGridFiles; i++) {
    std::cout << "characteristic length: " << gridfiles[i]
              << "\t#interface elements: " << numOfIElems[i]
              << " \terror curvature: " << errorsCurvature[i];
    if constexpr (dim == 2) {
  ...
  for (int i = 1; i < numOfGridFiles; i++) {
    const double EOC =
        log(errorsCurvature[i - 1] / errorsCurvature[i]) /
  ...
  if constexpr (dim == 2) {
    ...
    for (int i = 1; i < numOfGridFiles; i++) {
      const double EOC =
          log(errorsCenter[i - 1] / errorsCenter[i]) /
          log(characteristicLengths[i - 1] / characteristicLengths[i]);
      std::cout << "EOC: " << EOC << "\t(interface elements "
                << numOfIElems[i - 1] << " -> " << numOfIElems[i] << ")\n";
      assert(EOC > 1.6);
...
int main(int argc, char** argv) {
  try {
    ...
    using CurvatureOperatorElement =
        Dune::CurvatureOperator<IGridView, IMapper,
                                Dune::CurvatureLayout::Element>;
    using CurvatureOperatorVertex =
        Dune::CurvatureOperator<IGridView, IMapper,
                                Dune::CurvatureLayout::Vertex>;

    const std::string gridType = (dim == 2) ? "ellipse" : "sphere";

    // get interface grid view
    GridFactory gridFactory("grids/" + gridType + std::to_string(dim) +
                            "d.msh");
    ...
    for (const auto& iElem : elements(iGridView)) {
      ...
      errorCurvatureElement +=
          pow(curvaturesElement[iElemIdx] - exCurv, 2) * iVolume;

      if constexpr (dim == 2)
        errorCenterElement +=
            (centersElement[iElemIdx] - exactCenterOfCurvature(center))
                .two_norm2() *
            iVolume;
    ...
    for (const auto& vertex : vertices(iGridView)) {
      ...
      errorCurvatureVertex = std::max(
          errorCurvatureVertex, std::abs(curvaturesVertex[vertexIdx] - exCurv));

      if constexpr (dim == 2)
        errorCenterVertex =
            std::max(errorCenterVertex,
                     (centersVertex[vertexIdx] - exactCenterOfCurvature(center))
                         .infinity_norm());
    ...
    std::cout << "ErrorCurvatureElement: " << errorCurvatureElement
              << std::endl;
