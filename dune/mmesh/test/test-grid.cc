...
int main(int argc, char* argv[]) {
  ...
  GridFactory gridFactory((dim == 2) ? "grids/junction2d.msh"
                                     : "grids/plane3d.msh");
  ...
  if constexpr (verbose) {
    std::cout << "Rank " << grid.comm().rank() << ": Elements " << grid.size(0)
              << " (" << grid.ghostSize(0) << " ghost)" << std::endl;
    std::cout << "Rank " << grid.comm().rank() << ": Facets " << grid.size(1)
              << " (" << grid.ghostSize(1) << " ghost)" << std::endl;
    std::cout << "Rank " << grid.comm().rank() << ": Vertices "
              << grid.size(dim) << " (" << grid.ghostSize(dim) << " ghost)"
              << std::endl;

    std::cout << "Interface Rank " << grid.comm().rank() << ": Elements "
              << igrid.size(0) << " (" << igrid.ghostSize(0) << " ghost)"
              << std::endl;
    std::cout << "Interface Rank " << grid.comm().rank() << ": Vertices "
              << igrid.size(dim - 1) << " (" << igrid.ghostSize(dim - 1)
              << " ghost)" << std::endl;
  }
  ...
  auto computeVolume = [&](const auto& grid, bool interface = false) {
    ...
    if (grid.comm().rank() == 0)
      std::cout << " " << (!interface ? "Bulk" : "Interface") << " took " << dt
                << std::endl;
  };

  // Compute volume
  if (grid.comm().rank() == 0) std::cout << "Compute volume:" << std::endl;
  computeVolume(grid);
  ...
  if constexpr (verbose) {
    ...
    auto getConnectivity = [](const auto& grid) {
      ...
      for (const auto& e : elements(grid.leafGridView(), Partitions::all))
        connectivity[indexSet.index(e)] =
            (grid.partitionHelper().connectivity(e).size() > 0);
      return connectivity;
    };
