    ...
    for (auto e : elements(gridView)) {
      ...
      if (elementCount == 1) {
        ...
        checkProperties("boundary",
                        {{is0.boundary(), false}, {is1.boundary(), false}});
        ...
        const auto refElement =
            Dune::ReferenceElements<double, 1>::general(geo.type());
        ...
        checkProperties("reference element mapping",
                        {{vIdxGlobal1, 2u}, {vIdxGlobal2, 1u}});

        // try to obtain mmesh intersection
        const auto& bulkIs = mMesh.asIntersection(e);
        checkProperty("asIntersection", bulkIs.geometry().center(),
                      e.geometry().center());
      }
      elementCount++;
    }
