...
int main( int argc, char *argv[] )
{
  try {
    ...
    // print some status
    std::cout << "Create MMesh from grids/cube" + std::to_string(dim) + "d.dgf..." << std::endl;
    ...
    // print some status
    std::cout << "Write grid into dgf-grid-" + std::to_string(dim) + "d.vtu... " << std::endl;

    // write grid to vtk file
    Dune::VTKWriter<typename Grid::LeafGridView> vtkwriter( grid.leafGridView() );
    vtkwriter.write( "dgf-grid-" + std::to_string(dim) + "d", Dune::VTK::ascii );
